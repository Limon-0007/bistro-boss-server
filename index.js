    ...
    app.get("/users/admin/:email", verifyJwt, async (req, res) => {
      try {
        ...
        if (req.decoded.email !== email) {
          return res
            .status(401)
            .send({ error: true, message: "unauthorized admin", admin: false });
        }
        const query = { email: email };
        const user = await usersCollection.findOne(query);
        ...
        res.status(401).send({ error: true, message: "unauthorized admin" });
      }
    });
    ...
    app.get("/carts", verifyJwt, async (req, res) => {
      try {
        ...
        if (email !== decodedEmail) {
          return res
            .status(403)
            .send({ error: true, message: "request forbidden" });
        }
        const query = { email: email };
        const result = await cartCollection.find(query).toArray();
        res.send(result);
      } catch {
        res.status(403).send({ error: true, message: "request forbidden" });
      }
    });
